...
//
// Callers treat this as fire-and-forget: the user consented by clicking Accept,
// and persisting that is bookkeeping, so a failure must not cost them the take
// they just started. A failed POST means they are asked again next session,
...
export async function acceptRecordingConsent() {
  ...
  try {
    ...
    const error = new ApiError('Could not save your choice.', { status: 0, cause });
    logApiFailure(error, {
      surface: 'consent',
      method: 'POST',
      path: '/api/consent/recording',
  ...
  if (!response.ok) {
    const error = new ApiError('Could not save your choice.', {
